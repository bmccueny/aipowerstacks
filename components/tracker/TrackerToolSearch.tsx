...
export function TrackerToolSearch({
    ...
    return tools
      .filter(
        (t) =>
          ...
          !alreadyTracked.has(t.id),
      )
      .slice(0, 10);
  }, [search, tools, alreadyTracked]);
  ...
  const handleSearchKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.key === "Enter" &&
      highlightIdx >= 0 &&
      highlightIdx < filteredTools.length
    ) {
      e.preventDefault();
        ...
        <div ref={wrapperRef} className="relative">
          <div className="relative">
            ...
            <input
              ...
              aria-activedescendant={
                highlightIdx >= 0 ? `tool-option-${highlightIdx}` : undefined
              }
              className="w-full pl-10 pr-4 py-3 text-base sm:text-sm rounded-xl border border-border bg-background focus:outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
          {showDropdown && filteredTools.length > 0 && (
            <div
              ...
              role="listbox"
              className="absolute top-full left-0 right-0 z-[100] mt-2 bg-white dark:bg-neutral-900 border border-border rounded-xl shadow-2xl max-h-80 overflow-y-auto"
            >
          ...
          {showDropdown && search.length > 1 && filteredTools.length === 0 && (
            <div className="absolute top-full left-0 right-0 z-[100] mt-2 bg-white dark:bg-neutral-900 border border-border rounded-xl shadow-2xl px-4 py-3">
              <p className="text-sm text-muted-foreground">
                No results for &ldquo;{search}&rdquo; — try a different spelling
              </p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-5 p-3 bg-muted/50 rounded-xl">
            {selectedTool.logo_url ? (
              ...
              <span className="w-9 h-9 rounded-lg bg-primary/10 flex items-center justify-center text-sm font-bold text-primary">
                {selectedTool.name[0]}
              </span>
            )}
                ...
                <div className="p-4 rounded-xl border border-border">
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (customCost) handleAddSub(parseFloat(customCost));
                      }}
