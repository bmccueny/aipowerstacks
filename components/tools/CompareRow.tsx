...
const CELL_CN = 'min-w-[140px] sm:min-w-[180px] px-3 py-2.5 sm:px-4 sm:py-3 border-r border-border last:border-r-0'
...
export function BoolVal({ val }: { val: boolean }) {
  return val ? (
    ...
    <span className="inline-flex items-center justify-center h-5 w-5 sm:h-6 sm:w-6 rounded-full bg-muted/30 text-foreground/20">
...
export function SectionHeader({ label, maxTools }: { label: string; maxTools: number }) {
  return (
    <tr>
      <td
        colSpan={maxTools + 1}
        className="px-3 sm:px-4 py-2 bg-muted/40 dark:bg-muted/50 border-y border-border sticky left-0"
      >
...
export function CompareRow({ label, children, emptySlots, stripe = false, diff = false }: CompareRowProps) {
  return (
    <tr className={cn(
      ...
      stripe && 'bg-muted/20 dark:bg-muted/30'
    )}>
      {/* Sticky label cell */}
      <td className={cn(
