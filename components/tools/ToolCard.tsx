 ...
 * 💡 What: Wrapped ToolCard in React.memo with a custom comparison function.
 * 🎯 Why: In Next.js App Router, server-fetched data creates new object references on every navigation.
 *         This causes every ToolCard in lists/grids to re-render unnecessarily when navigating or applying filters,
 *         even if the tool data hasn't changed.
 * 📊 Impact: Prevents unnecessary React renders of 12-48 tool cards per page transition/filter change.
 *         Reduces main thread blocking time during hydration and client-side navigations.
 */
export const ToolCard = memo(ToolCardComponent, (prevProps, nextProps) => {
  return (
    ...
    // In Next.js App Router, server-fetched data creates new object references on every navigation.
    // Since tool data is mostly static on the frontend, checking the ID is the primary equality check.
    // However, we also check specific dynamic fields (like ratings) to ensure live updates reflect properly.
    ...
    prevProps.tool.model_provider === nextProps.tool.model_provider &&
    prevProps.tool.use_case === nextProps.tool.use_case &&
    prevProps.tool.has_api === nextProps.tool.has_api &&
    prevProps.tool.is_open_source === nextProps.tool.is_open_source &&
    prevProps.tool.has_mobile_app === nextProps.tool.has_mobile_app &&
    prevProps.tool.is_featured === nextProps.tool.is_featured &&
    prevProps.tool.pricing_tags?.[0] === nextProps.tool.pricing_tags?.[0] &&
    prevProps.tool.screenshot_urls?.[0] === nextProps.tool.screenshot_urls?.[0]
  );
});
