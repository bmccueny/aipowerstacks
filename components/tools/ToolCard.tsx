...
function arePropsEqual(prevProps: ToolCardProps, nextProps: ToolCardProps) {
  ...
  const prevKeys = Object.keys(prevProps.tool);
  const nextKeys = Object.keys(nextProps.tool);

  if (prevKeys.length !== nextKeys.length) return false;

  for (let i = 0; i < prevKeys.length; i++) {
    const key = prevKeys[i] as keyof ToolCardData;
    const prevVal = prevProps.tool[key];
    const nextVal = nextProps.tool[key];
