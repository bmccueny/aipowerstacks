...
export const metadata: Metadata = {
  title: 'AI Agents Guide 2026 | Automate Your Business',
  description: 'Learn how AI agents are transforming business productivity. Complete guide to building, deploying, and scaling AI agents.',
  alternates: { canonical: '/blog/ai-agents' },
}

export default async function AIAgentsClusterPage() {
  const [{ posts }] = await Promise.all([getPublishedPosts(1)])

  const agentPosts = posts.filter((p) =>
    p.tags?.some((t) => ['ai-agents', 'automation', 'workflow-automation', 'business-automation'].includes(t))
  )

  const relatedTopics = [
    { name: 'AI Productivity', slug: 'productivity', count: 6 },
    { name: 'LLM Comparison', slug: 'llm-comparison', count: 4 },
    { name: 'AI Memory', slug: 'ai-memory', count: 4 },
  ]
  ...
  const jsonLd = {
    ...
    description: 'Learn how AI agents are transforming business productivity',
    url: `${SITE_URL}/blog/ai-agents`,
    hasPart: agentPosts.map((p) => ({
      '@type': 'Article',
      headline: p.title,
    ...
    <>
      ...
      <div className="page-shell">
        <div className="text-center py-12 mb-8">
          ...
          <h1 className="text-4xl sm:text-5xl font-black tracking-tight mb-4">AI Agents Guide</h1>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            Learn how AI agents are transforming business productivity. Complete guide to building and scaling AI agents.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {agentPosts.slice(0, 6).map((post) => (
        ...
        <div className="glass-card rounded-2xl overflow-hidden border border-primary/15 mb-12">
          ...
          <div className="p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            <div className="sm:w-72 shrink-0">
              <NewsletterBanner source="agents-cluster" />
            </div>
          </div>
        </div>
      </div>
