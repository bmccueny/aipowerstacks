...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const pricingLabel = tool.pricing_model === 'free' ? 'Free' : tool.pricing_model === 'freemium' ? 'Freemium' : tool.pricing_model === 'paid' ? 'Paid' : tool.pricing_model === 'trial' ? 'Free Trial' : tool.pricing_model ?? 'N/A'
  const pricingBg = tool.pricing_model === 'free' ? '#065f46' : 'rgba(212, 64, 43, 0.2)'
  const pricingBorder = tool.pricing_model === 'free' ? '1px solid #0d9668' : '1px solid rgba(212, 64, 43, 0.35)'
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 32, color: '#ccc', textAlign: 'center', lineHeight: 1.4 }}>
          {tool.tagline}
        </div>
