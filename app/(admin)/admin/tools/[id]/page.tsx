...
export default async function EditToolPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase
    .from('tools')
    .select('id, name, slug, tagline, description, website_url, logo_url, category_id, pricing_model, pricing_details, use_case, team_size, integrations, status, is_verified, is_featured, is_supertools, is_editors_pick, model_provider, is_api_wrapper, wrapper_details, time_to_value, not_for, affiliate_url, affiliate_commission_pct')
    .eq('id', id)
    .single()
  ...
  const tool = data as {
    ...
    affiliate_url: string | null; affiliate_commission_pct: number | null
  }
