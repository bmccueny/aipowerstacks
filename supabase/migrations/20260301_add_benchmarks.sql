-- Migration: Add Real-Time Benchmarks to tools
-- Date: 2026-03-01

alter table public.tools add column if not exists api_latency integer; -- in milliseconds
alter table public.tools add column if not exists api_uptime numeric(5,2) default 100.00;
alter table public.tools add column if not exists last_benchmarked_at timestamptz;

-- Update search function to include these new columns
-- (We drop the function first because the return type is changing)
